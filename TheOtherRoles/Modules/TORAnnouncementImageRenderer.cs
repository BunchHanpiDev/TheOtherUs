...
namespace TheOtherRoles.Modules
{
	public class TORAnnouncementImageRenderer : MonoBehaviour
	{
		...
		internal static readonly Dictionary<IntPtr, TORAnnouncementImageRenderer> ActiveInstances = new();
		...
		internal void LayoutImages()
		{
			...
			// 计算文本实际内容区域的最低点
			float textBottom = _bodyText.textBounds.min.y;
			float fallbackY = textBottom - 0.2f;
			...
			foreach (var kvp in _imageRenderers)
			{
				...
				// 获取占位符信息（如果存在）
				bool hasPlaceholder = blocks.TryGetValue(kvp.Key, out var block);
				float maxHeight = hasPlaceholder ? block.Height : MaxImageHeight;
				if (maxHeight <= 0f) maxHeight = MaxImageHeight;

				// 计算缩放：只缩小，不放大，且高度不超过占位符高度或最大高度
				float scale = 1f;
				if (sw > maxWidth || sh > maxHeight)
				{
					scale = Mathf.Min(maxWidth / sw, maxHeight / sh);
				}
				scale = Mathf.Min(scale, MaxImageScale);

				rend.transform.localScale = new Vector3(scale, scale, 1f);
				...
				{
					// 有占位符，居中放置在占位符行区域内
					float yCenter = (block.Top + block.Bottom) * 0.5f;
					rend.transform.localPosition = new Vector3(leftEdge + scaledWidth * 0.5f, yCenter, 0f);
				}
				else
				{
					// 没有占位符，按顺序堆叠在文本下方
					rend.transform.localPosition = new Vector3(leftEdge + scaledWidth * 0.5f, fallbackY - scaledHeight * 0.5f, 0f);
					fallbackY -= scaledHeight + 0.2f;
					totalExtra += scaledHeight + 0.2f;
				}
			}
